<script>
    import { showSuggestions } from '../../utils/skills';
    ...
    async function submit_form() {
        sending = true;
        await update_rec('skill', skill);
        sending = false;
        offModels();
    }
    function show_sugg() {
        showSuggestions(skill.type, ref, setFun);
    }
...
>
    <div class=" mb-4">
        <h2 class="font-bold text-2xl capitalize">SKill</h2>
    </div>
    <form onsubmit={submit_form} class="m-auto w-fit grid grid-cols-1 place-content-center">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10 justify-center">
            ...
            <div class="flex flex-col gap-2">
                ...
                <select
                    ...
                    class="w-full rounded-lg border p-[8px] focus:border-2 transition"
                    name="course"
                >
        ...
        <div class="w-full">
            <div bind:this={ref} id="chap-o" class="flex flex-wrap gap-2"></div>
        </div>
